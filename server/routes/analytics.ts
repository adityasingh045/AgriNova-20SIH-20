...
function generateNDVI(fieldId: string, points = 12): NDVITimeseries {
  ...
  const series = Array.from({ length: points }, (_, i) => {
    ...
    return {
      t: months[(start + i + 12) % 12],
      v: Math.max(0, Math.min(1, base + noise)),
    };
  });
...
export const analyzeImage: RequestHandler = async (req, res) => {
  ...
  const seedStr =
    (body.imageUrl || body.imageData || "no-image") +
    (body.fieldId || "field-a");
  ...
  const pestRisk: ImageAnalysisResult["pestRisk"] =
    healthScore > 0.7 ? "low" : healthScore > 0.5 ? "medium" : "high";
  const result: ImageAnalysisResult = {
    ...
    notes: [
      ...
      pestRisk === "high"
        ? "Pest risk elevated; inspect hotspots."
        : pestRisk === "medium"
          ? "Watch conditions over next week."
